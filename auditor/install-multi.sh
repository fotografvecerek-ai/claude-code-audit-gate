...
for p in "${PLAN[@]}"; do IFS='|' read -r repo name prof gh port dirty ng <<< "$p"
  ...
  if [ "$prof" != "JEN_AUDIT" ]; then ( cd "$repo" && PRE="$WS/AUDIT/.pre-install-status.txt"; OURS=(); while IFS= read -r l; do f="${l:3}"; f="${f#\"}"; f="${f%\"}"; [ -n "$f" ] || continue; if [ -f "$PRE" ] && grep -qF -- " $f" "$PRE"; then continue; fi; OURS+=("$f"); done < <(git status --porcelain --untracked-files=all -- $(for f in .claude .github .gitattributes .gitignore CLAUDE.md; do [ -e "$f" ] && printf '%s ' "$f"; done) 2>/dev/null); [ ${#OURS[@]} -gt 0 ] && { git add -- "${OURS[@]}"; git commit -q -m "chore(audit): instalace auditora" -- "${OURS[@]}"; }; n=$(git status --porcelain | grep -c .); [ "$n" -gt 0 ] && echo "  v projektu zůstává $n souborů neuložených v gitu (bylo tak už před instalací) — auditor to zapíše jako první nález." ); fi
