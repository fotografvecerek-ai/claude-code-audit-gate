...
try {
  ...
  const role = val('--for'), ev = val('--event') || 'post';
  const ws = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..'); const bus = path.join(ws, 'AUDIT', 'bus');
  ...
  const save = () => { const ids = [...seen, ...fresh.map(x => x.f)].slice(-2000); fs.writeFileSync(stF, JSON.stringify({ mtime: mt, ids })); };
  ...
  if (ev === 'stop') { if (input.stop_hook_active) process.exit(0); save(); process.stderr.write(msg + '\nNež skončíš tah, zprávy vyřiď (odpověz, potvrď nebo zařaď).\n'); process.exit(2); }
